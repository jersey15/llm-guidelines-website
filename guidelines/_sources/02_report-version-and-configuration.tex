...
LLMs (especially when used as a service) are frequently updated, and different versions may produce varying results for the same input.
Moreover, the model configuration and parameters influence the output generation of the models.
It is crucial to document the specific version of the LLM used in the study, along with the date when the experiments were conducted, and the exact configuration being used. \comment{the date should be irellevant if one tracks model version and parameters. no?}
Furthermore, detailed documentation of the configuration and parameters used during the study is necessary for reproducibility. \comment{duplicate with previous sentence which talks also about configuration. try to consolidate? also because version and configuration are both together relevant for reproducibility}
Our recommendations is to report: \comment{unfinished sentence?}
Additionally, a thorough description of the hosting environment of the LLM or LLM-based tool should be provided \comment{when possible}, especially in studies focusing on performance or any time-sensitive measurement. \comment{is this not a bit trivial; sure if you're studying performance you will report on the hosting. I wonder if one argument might be that you want to report the hardware on which you ran the LLM because that might also impact the outputs in some cases - I refer to that in section 5 - but we could also move it here if you think it would make sense }
